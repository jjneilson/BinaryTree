...
/*****And now for the big tree!!!  Part 6 *************/
	TreeInterface *studtree = new TreeInterface("StudentAnswers.txt");
		cout <<"************************************" << endl;
	TreeInterface *joketree = new TreeInterface("Jokes.txt");
	joketree->Interface();

	bstNode *person = studtree->tree->find("Yarrington","Debra");
	person->printNode();
	person = studtree->tree->find("Neilson","John");
	person->printNode();
	person = studtree->tree->find("Abhishek","Kumar");
	person->printNode();
	person = studtree->tree->find("Poltergeist","Penelope");
	person->printNode();
	person = studtree->tree->remove("Grotesque","Gary");
	person->printNode();
	person = studtree->tree->find("Grotesque","Griselda");
	person->printNode();
	studtree->tree->insert("Grotesque","Gary",1,"Hello");
	person = studtree->tree->find("Grotesque","Gary");
	person->printNode();
	person = studtree->tree->find("Grotesque","Griselda");
	person->printNode();
	person = studtree->tree->find("Felker","Blair");
	person->printNode();
	person = studtree->tree->find("Hart","William");
	person->printNode();
	person = studtree->tree->find("Gwin","Mathew");
	person->printNode();
